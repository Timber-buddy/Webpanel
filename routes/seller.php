...
Route::group(['prefix' => 'seller', 'as' => 'seller.'], function () {
    Route::post('razorpay/callback', [ProfileController::class, 'process_payment'])->name('razorpay.callback');
    Route::get('planDetails/{id}', [ProfileController::class, 'planDetails'])->name('plan.details');
    Route::get('razorpay/process-as-failed', [ProfileController::class, 'process_payment_as_failed'])->name('process.payment.failed');
});
...
Route::group(['namespace' => 'App\Http\Controllers\Seller', 'prefix' => 'seller', 'middleware' => ['seller', 'verified', 'user', 'prevent-back-history'], 'as' => 'seller.'], function () {
    Route::controller(DashboardController::class)->group(function () {
        ...
        Route::post('/upgrade-package', 'package_upgrde')->name('upgrade.package');
    });

    // Quotations
    Route::controller(QuotationController::class)->group(function () {
        Route::get('/quotation/all', 'index')->name('quotation.all');
        Route::get('/quotation/view/{id?}', 'view')->name('quotation.view');
        Route::post('/quotation/mail', 'mail')->name('quotation.mail');
        Route::DELETE('/quotation/destroy/{id}', 'destroy')->name('quotation.destroy');
    });

    // Seller Roles and Permission
    Route::controller(StaffController::class)->group(function () {
        Route::get('/staff/all', 'index')->name('staff.all');
        Route::get('/staff/add', 'add')->name('staff.add');
        Route::post('/staff/store', 'store')->name('staffs.store');
        Route::get('/staff/destroy/{id}', 'destroy')->name('staffs.destroy');
        Route::get('/staff/{id}/edit', 'edit')->name('staffs.edit');
        Route::patch('/staff/update/{id}', 'update')->name('staffs.update');

        Route::get('/staff/permission', 'permission')->name('staff.permissions');
    });
    ...
    Route::controller(ProductQueryController::class)->group(function () {
        ...
        Route::get('/product-queries/edit/{id}', 'edit')->name('product_query.edit');
        Route::get('/product-queries/delete/{id}', 'delete')->name('product_query.destroy');
