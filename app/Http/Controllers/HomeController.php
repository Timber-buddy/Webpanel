...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (session()->has('selected_location'))
        {
            ...
            {
                $userIds = \App\Models\Address::where('city_id', session()->get('selected_location'))->pluck('user_id')->toArray();
                $userIds = array_values(array_unique($userIds));

                $newest_products = Product::where('published', '1')->where('auction_product', 0)->where('approved', '1')->whereIn('user_id', $userIds)->latest()->limit(12)->get();
            }
            else
            {
                $newest_products = Product::where('published', '1')->where('auction_product', 0)->where('approved', '1')->latest()->limit(12)->get();
            }
        }
        else
        {
    ...
    public function userProfileUpdate(Request $request)
    {
        ...
        if ($request->new_password != null && ($request->new_password == $request->confirm_password)) {
            ...
            $body = "🔒 Password Change Successful! 🔒<br>
                We wanted to let you know that the password for your account on ".env('APP_NAME')." has been successfully updated.<br>
                If you did not change your password or believe this might be an error, please contact our Support Team through the app";
            ...
            sendNotification($user->id, "change_password", null, null, null, $body);
        }
        ...
        if (($user = User::where('email', $request->email)->where('verification_code', $request->code)->first()) != null) {
            if ($request->password == $request->password_confirmation) {
                ...
                else
                {
                    ...
                    sendNotification(auth()->user()->id, "change_password", null, null, null, $body);
                }
                return redirect()->route('home');
            } else {
    ...
    public function quotation_view(Request $request)
    {
        ...
        $quotation->customer_id = Auth::user()->id;
        $quotation->seller_id = $request->sellerid;
        ...
        $attributeContainers = [];
        foreach ($requestData['attributes'] as $attribute) {
            if (isset($attribute['attribute'])) {
                $attributeContainers[] = $attribute['attribute'];
            }
        }
        foreach ($attributeContainers as $attributeValue) {
            $quotationAttribute = new QuotationAttribute();
            $quotationAttribute->quotation_id = $quotation->id;
            $quotationAttribute->attribute = $attributeValue;
            $quotationAttribute->save();
        }

        //  Quotation Attribute Data Table
        $messagesContainers = [];
        foreach ($requestData['attributes'] as $attribute) {
            if (isset($attribute['messagesContainer'])) {
                $messagesContainers[] = $attribute['messagesContainer'];
            }
        }
        foreach ($messagesContainers as $attributeValue) {
            $quotationAttributeData = new QuotationAttributeData();
            $quotationAttributeData->quotaton_id = $quotation->id;
            $quotationAttributeData->attribute_data = $attributeValue;
            $quotationAttributeData->save();
        }

        $user = User::find($request->sellerid);
        if (!is_null($user))
        {
            $product = Product::find($request->productid);
            if($user->user_type == "seller")
            {
                $body = "📜 New Quotation Request!<br>
                    A customer is interested in ".$product->name.". Tap to view the details and respond.";
                sendSellerNotification($request->sellerid, 'seller_product_quotation', null, $quotation->id, null, $body);
            }
            else
            {
                $admin = User::where('user_type', 'admin')->first();
                $body = "🛒 Quotation Alert! <br>
                    A quotation for ".$product->name." has been requested. Tap to review the request details.";
                sendAdminNotification($admin->id, 'admin_product_quotation', null, $quotation->id, null, $body);
            }
        }

        $body = "✅ Quotation Submitted!<br>
            Your quotation for ".$product->name." has been successfully placed.<br>
            Tap to view details or track status.";
        sendNotification(Auth::user()->id, 'product_quotation', null, $quotation->id, null, $body);

        return response()->json($messagesContainers, 200);
    }
