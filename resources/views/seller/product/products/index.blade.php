...
@section('panel_content')
    <style>
        input[type="checkbox"]:disabled+.slider {
            ...
            /* Change the background color to a grayish tone */
            cursor: not-allowed;
            ...
            @if (!is_null($subscription) || !empty($subscription))
                <div class="col-md-4">
                    <div class="bg-grad-1 text-white rounded-lg overflow-hidden">
                        ...
                        {{-- <div class="px-3 pt-3 pb-3" id="remaining-products-container">
                        ...
                        <div class="px-3 pt-3 pb-3">
                            <div class="h4 fw-700 text-center" id="remaining-products-container">
                                {{ max(0, $subscription->plan->product_limit - $product_count) }}</div>
                            <div class="opacity-50 text-center">{{ translate('Remaining Publish Products') }}</div>
                        </div>
                    </div>
                </div>
...
@section('script')
    <script type="text/javascript">
        ...
        //              el.checked = !el.checked; // Revert the checkbox state if the update fails
        //         }
        //         else{
        //             AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
        //             // location.reload();
        ...
        function update_published(el) {
            ...
            $.post('{{ route('seller.products.published') }}', {
                ...
                if (data == 1) {
                    AIZ.plugins.notify('success', '{{ translate('Published products updated successfully') }}');
                    var remainingProductsElement = $('#remaining-products-container');
                    var remainingProducts = parseInt(remainingProductsElement.text());
                    remainingProductsElement.html(`
                    ${remainingProducts ? (el.checked ? remainingProducts - 1 : remainingProducts + 1) : remainingProducts + 1}
                `);
                } else if (data == 2) {
                    AIZ.plugins.notify('danger', '{{ translate('Please upgrade your package.') }}');
                    el.checked = !el.checked; // Revert the checkbox state if the update fails
