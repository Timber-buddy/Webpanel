@php
    if (session()->has('selected_location'))
    {
        if (session()->get('selected_location') != "all")
        {
            ...
            $featured_products = \App\Models\Product::where('featured', '1')->where('published', '1')->where('auction_product', 0)->where('approved', '1')->whereIn('user_id', $userIds)->latest()->limit(12)->get();
        }
        else
        {
            $featured_products = \App\Models\Product::where('featured', '1')->where('published', '1')->where('auction_product', 0)->where('approved', '1')->latest()->limit(12)->get();
        }
    }
    else
    {
