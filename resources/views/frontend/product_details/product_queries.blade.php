...
<div class="bg-white border mt-4 mb-4" id="product_query">
    ...
    <!-- Login & Register -->
    @guest
    <p class="fs-14 fw-400 mb-0 px-3 px-sm-4 mt-3"><a href="{{ route('user.login') }}">{{ translate('Login') }}</a> {{ translate('or') }} <a class="mr-1" href="{{ route('user.registration') }}">{{ translate('Register ') }}</a>{{ translate(' to submit your questions to seller') }}
    </p>
    @endguest
    ...
    <div class="query form px-3 px-sm-4">
        ...
        <form action="{{ route('product-queries.store') }}" method="POST">
            ...
            <div class="form-group">
                <textarea class="form-control rounded-0" rows="3" cols="40" name="question" placeholder="{{ translate('Write your question here...') }}" style="resize: none;"  maxlength="500"></textarea>
    ...
    <div class="question-area my-4 mb-0 px-3 px-sm-4">
        ...
        <div class="produc-queries mb-4">
            ...
            <div class="d-flex my-2">
                @if(is_null($product_query->reply) || $product_query->reply == "")
                <a class="btn btn-icon btn-success btn-sm" href="javascript:void(0);" title="{{ translate('Edit') }}" onclick="showDiv('edit-query-{{$product_query->id}}')">
                    <i class="las la-edit"></i>
                </a>&emsp;
                @endif
                <a class="btn btn-icon btn-danger btn-sm" href="{{route('front.product-query.destroy', encrypt($product_query->id))}}" title="{{ translate('Delete') }}">
                    <i class="las la-trash"></i>
                </a>
            </div>
            <div style="display: none;" id="edit-query-{{$product_query->id}}">
                <form action="{{ route('front.product-query.update', encrypt($product_query->id)) }}" method="POST">
                    @csrf
                    <div class="form-group">
                        <textarea class="form-control rounded-0" rows="3" cols="40" name="question" placeholder="{{ translate('Write your question here...') }}" style="resize: none;">{{ strip_tags($product_query->question) }}</textarea>

                    </div>
                    <button type="submit" class="btn btn-sm w-150px btn-primary rounded-0">{{ translate('Update') }}</button>
                    <button type="button" class="btn btn-sm w-150px btn-primary rounded-0" onclick="closeDiv('edit-query-{{$product_query->id}}')">{{ translate('Close') }}</button>
                </form>
            </div>
        </div>
        @endforeach
