        ...
        <div class="card">
            ...
            <div class="card-body">
                <form class="form-horizontal" action="{{ route('profile.update', Auth::user()->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 col-from-label" for="confirm_password">{{translate('City')}}</label>
                        <div class="col-sm-9">
                                    ...
                                    @foreach($locations as $location)
                                        @if(!is_null($location->cities))
                                            @foreach($location->cities as $city)
                                                <option value="{{$city->id}}" @isset(Auth::user()->city) @if (Auth::user()->city == $city->id) selected @endif @endisset>
