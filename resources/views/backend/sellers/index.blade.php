                ...
                @foreach($shops as $key => $shop)
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                            {{ $shop->verification_status == 1 ? 'Verified' : 'Non-Verified' }}
                            @endif
                        </td>
                        <td>
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                    @can('view_seller_profile')
                                        <a href="javascript:void(0);" onclick="show_seller_profile('{{$shop->id}}');"  class="dropdown-item">
                                            {{translate('Profile')}}
                                        </a>
                                    @endcan
                                    @can('login_as_seller')
...
@section('script')
    <script type="text/javascript">
        ...
        function verifyGstNumber(gst_number, id)
        {
            $("#auto-load").show();
            $("#gst-verify-btn").hide();
            var url = "{{config('gst.number_check_url')}}";

            $.get(url+'/'+gst_number, function(result){
                $("#auto-load").hide();

                if (result.flag == false)
                {
                    var html = `<svg xmlns="http://www.w3.org/2000/svg" width="17.5" height="17.5" viewBox="0 0 17.5 17.5">
                    ...
                    $('#gst_number_verify_status').empty().html(html + result.message);

                    $("#gst-verify-btn").show();
                    updateGSTStatus(id, result.message);
                }
                else
                {
                    var html = `<svg xmlns="http://www.w3.org/2000/svg" width="17.5" height="17.5" viewBox="0 0 17.5 17.5">
                    ...
                    {
                        $('#gst_number_verify_status').empty().html(html + " Verified");
                        $("#gst-verify-btn").remove();
                        updateGSTStatus(id, "Verified");
                    }
                    else
                    {
                        $('#gst_number_verify_status').empty().html(html + " Verified but not active");
                        $("#gst-verify-btn").remove();
                        updateGSTStatus(id, "Verified but not active");
                    }
                }
            });
        }
        ...
        {
            url = "{{route('sellers.update_gst_status')}}";
            $.post(url, {"_token": "{{ csrf_token() }}", "id": id, "status": status}, function(result){
                console.log(result);
            });
        }
