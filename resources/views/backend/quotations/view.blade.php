...
<div class="aiz-titlebar text-left mt-2 mb-3">
    ...
    <div class="card p-4">
        ...
        @endphp
        @if(!empty($quotations))
            @foreach($quotations as $key => $quteMsg)
                <div class="card-body">
                            ...
                            <div class="media mb-2">
                                <div class="media-body">
                                    <h6 class="mb-0 fw-600">
                                        @php
                                            $user = \App\Models\User::where('id', $quteMsg->user_id)->first();
                                        @endphp

                                        @if(auth()->check() && in_array($user->user_type, ['staff','admin']))
                                            <span class="float-left">
                                                <br>
                                                {{ $user->name }}
                                                <p class="opacity-50">{{ $quteMsg->created_at->format('F j, Y g:i A') }}</p>
                                                {{ $quteMsg->message }}
                                            </span>
                                        @else
        ...
        @can('quotation_reply')
            <form class="pt-4" action="{{ route('quotation.mail') }}" method="POST">
            @csrf
